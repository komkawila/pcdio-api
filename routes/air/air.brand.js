...
route.get('/:brand_id', async (req, res, next) => {
    ...
    await db.query("SELECT * FROM brand_tb WHERE brand_id = ?", [brand_id], function (err, result, fields) {
        if (err) {
            console.log(err);
            res.send({ err: true, message: err });
        } else {
...
route.delete('/:brand_id', async (req, res, next) => {
    ...
    const brand_id = req.params.brand_id;
    await db.query("DELETE FROM brand_tb WHERE brand_id = ?", [brand_id], function (err, result, fields) {
        if (err) {
            console.log(err);
            res.send({ err: true, message: err });
        } else {
...
route.put('/', async (req, res, next) => {
    ...
    const brand_name = req.body.brand_name;
    await db.query("UPDATE brand_tb SET brand_name = ? WHERE brand_id = ?", [brand_name, brand_id], function (err, result, fields) {
        if (err) {
            console.log(err);
            res.send({ err: true, message: err });
        } else {
