...
// expire 
route.patch('/expire/:device_id', async (req, res, next) => {
    const device_id = req.params.device_id;
    const value = req.body.value;
    await db.query("UPDATE device_tb SET expire = ? WHERE device_id = ?",
        [value, device_id],
        function (err, result, fields) {
            if (err) {
                console.log(err);
...
// expire_price 
route.patch('/expireprice/:device_id', async (req, res, next) => {
    const device_id = req.params.device_id;
    const value = req.body.value;
    await db.query("UPDATE device_tb SET expire_price = ? WHERE device_id = ?",
        [value, device_id],
        function (err, result, fields) {
            if (err) {
                console.log(err);
